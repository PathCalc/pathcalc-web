...
export function MobileDrawer({ children }: { children: ReactNode }) {
  ...
  return (
    <Drawer
      ...
      handleOnly={true}
    >
      <DrawerContent className="h-dvh z-40 max-w-[400px] mx-auto shadow-[0_-2px_4px_2px_rgba(0,0,0,0.1),0_-2px_4px_-1px_rgba(0,0,0,0.06)] bg-gray-50">
        <ScrollArea className="h-[calc(100dvh-var(--snap-point-height)-30px)]">
          <div className="pt-5 pb-10 px-5 ">{children}</div>
        </ScrollArea>
      </DrawerContent>
    </Drawer>
  );
